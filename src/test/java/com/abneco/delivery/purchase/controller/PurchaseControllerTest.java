...
public class PurchaseControllerTest {
    ...
    void test_register_purchase() throws Exception {
        ...
        PurchaseForm form = new PurchaseForm(buyer.getId(), List.of(product.getId()), QUANTITY);
        mockMvc.perform(post("/purchase")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(form)))
                .andExpect(status().isCreated());

        Purchase purchase = repository.findByBuyerId(buyer.getId()).orElseThrow(() -> new ResourceNotFoundException("Purchase not found."));
